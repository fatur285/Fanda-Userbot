...
@fanda_cmd(pattern='yatim(?: |$)(.*)')
async def typewriter(typew):
    typew.pattern_match.group(1)
    sleep(2)
    xx = await edit_or_reply(typew, "`Hai anak yatim, Jangan Lupa Makan Yaa`")
    sleep(2)
    await xx.edit("`Tar bokap lu di surga kecewa, emak lu udh masak malah kaga dimakan sama anaknya`")
    sleep(2)
    await xx.edit("`Apa mau disuapin sama bokap gua?`")
    sleep(2)
    await xx.edit("`eitss, G dlu cari bapak sendiri sana!`")
    sleep(2)
    await xx.edit("`Makanya bapak tuh dijaga jangan dibiarin kliaran malem²`")
    sleep(2)
    await xx.edit("`KETAWAIN DULU AE`")
    sleep(2)
    await xx.edit("`HAHAHAHAHAHAHA`")
    sleep(2)
    await xx.edit("`KASIAN GA PUNYA BAPAK`")
    sleep(2)
    await xx.edit("`CANDAA YATIM`")
# Create by myself @localheart


@fanda_cmd(pattern="kont(?: |$)(.*)")
async def typewriter(typew):
    typew.pattern_match.group(1)
    sleep(1)
